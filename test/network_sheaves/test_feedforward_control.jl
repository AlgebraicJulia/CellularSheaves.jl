...
@testset "Feedforward Control Abstractions" begin
    ...
    @testset "FeedforwardLQRController & FeedforwardAgentState" begin
        dyn = PlanarQuadrotorDynamics()
        dt = 0.05
        Q = Matrix{Float64}(I, 6, 6) * 100.0
        R = Matrix{Float64}(I, 2, 2) * 0.01
        
        ctrl = FeedforwardLQRController(dyn, dt, Q, R)
        @test ctrl isa AbstractAgentController
        @test size(ctrl.Bc_pinv) == (2, 6)

        x0 = zeros(6)
        state = FeedforwardAgentState(x0, dyn, dt, ctrl, 0.02)
        ...
        @test length(x_act) == 6
        @test length(x_ref) == 6
        @test norm(x_act) > 0.0 # Agent moved using feedback + feedforward
    end
end

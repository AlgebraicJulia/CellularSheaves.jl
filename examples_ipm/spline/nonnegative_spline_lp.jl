...
function run_lp_benchmark(; optimizer = nothing, dual_optimizer = nothing, nwarmup::Int = 2, nruns::Int = 5)
    optimizer === nothing && error("Pass optimizer, e.g. run_lp_benchmark(optimizer=Mosek.Optimizer)")

    cases = [
        (:nonneg,   50, 6),   # M=50 pieces, degree 6
        (:nonneg,   100, 6),
        (:monotone, 50, 6),
        (:monotone, 100, 6),
        (:convex,   50, 6),
    ...
    println("="^80)
    println("Shape-Spline LP Benchmark: Sheaf IPM vs Mosek vs Mosek-D")
    println("="^80)
    println()
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    run_lp_benchmark(; optimizer, dual_optimizer,

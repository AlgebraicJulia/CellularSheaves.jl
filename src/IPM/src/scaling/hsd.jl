...
closed-form scalar `d₂` that equilibrates the `[f; g]` border row/column to unit
∞-norm, applies it in place (`f .*= d₂`, `g .*= d₂`), and stores it in
`scaling.bscl`. The embedding's corner entry is zero, so the general formula
`d₂ = min(1/‖[f;g]‖∞, 1/√|γ|)` reduces to `d₂ = 1/‖[f;g]‖∞`.

...
function cdot3(p::AbstractVector, d::AbstractVector)
    @inbounds pr1, e1 = twoprod(p[1], d[1])
    @inbounds pr2, e2 = twoprod(p[2], d[2])
    @inbounds pr3, e3 = twoprod(p[3], d[3])

    s2, se2 = twosum(pr1, pr2)
    s3, se3 = twosum(s2, pr3)

    return s3 + (e1 + e2 + e3 + se2 + se3)
end

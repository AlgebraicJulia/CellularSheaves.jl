...
function sdpshadow!(s::AbstractVector{T}, x::AbstractVector{T}, wrk::ConeWorkspace{T}) where {T}
    ...
    d == 1 && return sdpshadow1!(s, x, M, Z)
    d == 2 && return sdpshadow2!(s, x, M, Z)
    d == 3 && return sdpshadow3!(s, x, M, Z)
    d == 4 && return sdpshadow4!(s, x, M, Z)

    return sdpshadow!(s, x, M, Z)
end

function sdpshadow1!(s::AbstractVector{T}, x::AbstractVector{T}, ::AbstractMatrix{T}, ::AbstractMatrix{T}) where {T}
    @inbounds x[1] > 0 || return false
    @inbounds s[1] = inv(x[1])
    return true
end

function sdpshadow2!(s::AbstractVector{T}, x::AbstractVector{T}, M::AbstractMatrix{T}, Z::AbstractMatrix{T}) where {T}
    @inbounds smat!(M, x)
    @inbounds chol2!(Symmetric(M, :L)) || return false
    @inbounds copyto!(Z, I)
    @inbounds rdiv2!(Z, LowerTriangular(M)')
    @inbounds rdiv2!(Z, LowerTriangular(M))
    @inbounds svec!(s, Z)
    return true
end

function sdpshadow3!(s::AbstractVector{T}, x::AbstractVector{T}, M::AbstractMatrix{T}, Z::AbstractMatrix{T}) where {T}
    @inbounds smat!(M, x)
    @inbounds chol3!(Symmetric(M, :L)) || return false
    @inbounds copyto!(Z, I)
    @inbounds rdiv3!(Z, LowerTriangular(M)')
    @inbounds rdiv3!(Z, LowerTriangular(M))
    @inbounds svec!(s, Z)
    return true
end

function sdpshadow4!(s::AbstractVector{T}, x::AbstractVector{T}, M::AbstractMatrix{T}, Z::AbstractMatrix{T}) where {T}
    @inbounds smat!(M, x)
    @inbounds chol4!(Symmetric(M, :L)) || return false
    @inbounds copyto!(Z, I)
    @inbounds rdiv4!(Z, LowerTriangular(M)')
    @inbounds rdiv4!(Z, LowerTriangular(M))
    @inbounds svec!(s, Z)
    return true
end

function sdpshadow!(s::AbstractVector{T}, x::AbstractVector{T}, M::AbstractMatrix{T}, Z::AbstractMatrix{T}) where {T}
    @inbounds smat!(M, x)

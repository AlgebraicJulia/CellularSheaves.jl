...
# are passed in, not owned (as in solveuzw!). btol is the backward-error stop (bkwerr ≤ btol); we pass 0
# because the pricing model uses an absolute residual tolerance (atol). The knob costs one comparison per
# iteration (bkwerr is computed anyway for the machine-precision stop), so it is kept.
...
struct CraigWorkspace{T}
    ...
    Btu::FVector{T}    # Bᵀ u (n)
    v::FVector{T}      # (F Fᵀ)⁻¹ N v (n)
    w::FVector{T}      # y-recurrence direction (m)
    u::FVector{T}      # Golub–Kahan u-side (m)  [ = M u, M = I ]
    Bv::FVector{T}     # B v (m)
end
...
function CraigWorkspace{T}(m::Integer, n::Integer) where {T}
    return CraigWorkspace{T}(FVector{T}(undef, n), FVector{T}(undef, n), FVector{T}(undef, n),
                             FVector{T}(undef, m), FVector{T}(undef, m), FVector{T}(undef, m))
end

function craig!(wrk::CraigWorkspace{T}, B, F, divwrk, x, y, b; kwargs...) where {T}
    return craig!(wrk.Nv, wrk.Btu, wrk.v, wrk.w, wrk.u, wrk.Bv, B, F, divwrk, x, y, b; kwargs...)
end
...
function craig!(
    ...
    fill!(y, zero(T))
    #
    # u ← b / ‖b‖  (u ≡ M u, M = I); x = 0 is a zero-residual solution when b = 0
    #
    copyto!(u, b)
    β₁ = norm(u)
    nr = β₁
    ...
    while !(solved || inconsistent || illcond || iter ≥ itmax)
        ...
        if iszero(α)
            ...
            ξ = -θ / ρ * ξ
            axpy!(ξ, v, x)                     # x ← x + ξ v
            axpby!(one(T), u, -θ / ρprv, w)    # w ← u − (θ/ρprv) w

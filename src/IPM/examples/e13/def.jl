# =============================================================================
# e13/def.jl — Distributed sound-zone control: cooperative multizone
...
#   julia --project e13/run.jl --mosek      # + Mosek
#   julia --project e13/run.jl --quick      # quick (smaller) sweep
#   julia --project e13/run.jl --joint      # per-ball joint SOCs (ablation)
#   julia --project e13/run.jl --nobudget   # drop amplifier caps (ablation)
#   julia --project e13/run.jl --ldial      # block-size dial Lf at fixed Z
#
...
function run()
    println("\n", "=" ^ 78)
    println("  E13: distributed sound zones, cooperative rendering ",
    ...
    for sz in (LDIAL ? lsizes() : zsizes())
        ...
        m_msk = msk_opt !== nothing ?
            measure_jump(() -> first(build_jump_soundzone(probk, ctxk, msk_opt)); nruns = NRUNS) :

...
print(f"HSD CEILING RFE (d_hi): {len(D)} IPM rows, {CF['n_families']} families, {len(FULL)} start features",flush=True)
...
json.dump([(n,round(m,4),fs) for n,m,fs in curve],open(os.path.join(HERE,"phase2_rfe_hsd_ceiling.json"),"w"),indent=1)

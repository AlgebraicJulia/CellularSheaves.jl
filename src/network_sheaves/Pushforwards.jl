...
function fiber_section_basis(F, verts::Vector{Int}, fedges::Vector{Tuple{Int,Int}})
    ...
    if isempty(fedges)
        return Matrix{Float64}(I, total_dim, total_dim)
    end
...
function pushforward_sheaf(hom::GraphHomomorphism, F)
    ...
    # Group cross-edges by their target edge pair (tri_u, tri_v) with tri_u < tri_v
    # to handle non-injective edge maps correctly.
    ...
    for (e, tri_u, tri_v) in cross_edges(hom, g)
        ...
        # Use canonical key (min, max) to group edges consistently.
        key = (min(tri_u, tri_v), max(tri_u, tri_v))
        # Store restriction maps in (tri_u, tri_v) orientation.
        if !haskey(edge_groups, key)
            edge_groups[key] = Tuple{Matrix{Float64},Matrix{Float64}}[]
        end
        # If the canonical order differs from (tri_u, tri_v), swap the maps.
        if key == (tri_u, tri_v)
    ...
    for ((p, q), rms) in edge_groups
        # Direct sum: vertically stack all restriction maps.

...
function controlled_vehicle_platoon_with_herding(
    ...
    # Helper functions for vertex indexing in the ladder graph
    # Vehicle 1 vertices: 1 to k+2
    v1_state(t) = t                              # 1 to k+1: state vertices
    v1_ctrl(t) = k+2 + t                         # k+3 to 2k+2: control vertices (but we don't use these directly)
    
    # Vehicle 2 vertices: k+3 to 2(k+2)
    v2_state(t) = (k+2) + t                      # k+3 to 2k+3: state vertices  
    v2_ctrl(t) = 2*(k+2) + t                     # 2k+4 to 3k+3: control vertices (but we don't use these directly)
    
    # Actually, let's follow the exact pattern from ControlledTrajectorySheaf:
    # For a single vehicle with k time steps:
    # - Vertex 1: x₁ (n-dim, dummy initial state)
    # - Vertices 2 to k+1: (xₜ, uₜ) ((n+m)-dim, state-control pairs)
    # - Vertex k+2: x_{k+1} (n-dim, terminal state)
    
    # So for our ladder graph:
    # Vehicle 1: vertices 1 to k+2
    #   Vertex 1: x₁⁽¹⁾ (n-dim)
    #   Vertices 2 to k+1: (xₜ⁽¹⁾, uₜ⁽¹⁾) ((n+m)-dim)
    #   Vertex k+2: x_{k+1}⁽¹⁾ (n-dim)
    #
    # Vehicle 2: vertices k+3 to 2(k+2)
    #   Vertex k+3: x₁⁽²⁾ (n-dim)
    #   Vertices k+4 to 2k+3: (xₜ⁽²⁾, uₜ⁽²⁾) ((n+m)-dim)
    #   Vertex 2(k+2): x_{k+1}⁽²⁾ (n-dim)
    
    # Backbone edges for vehicle 1 (exactly as in ControlledTrajectorySheaf)
    # Edge (1,2): connects x₁⁽¹⁾ dummy to (x₁⁽¹⁾, u₁⁽¹⁾)
    ...
    # Edge (k+2, k+3): connects x₁⁽²⁾ dummy to (x₁⁽²⁾, u₁⁽²⁾)

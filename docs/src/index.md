...
`CellularSheaves.jl` is a Julia library for constructing, analyzing, and computing with cellular sheaves on graphs. It provides efficient sparse solvers for sheaf Laplacians, global sections, harmonic extension, and sheaf-theoretic formulations of dynamical systems and control problems.

The package is part of the [AlgebraicJulia](https://algebraicjulia.org) ecosystem and implements computational methods developed by Tyler Hanks, Hans Riess, Matthew Hale, Warren Dixon, and James Fairbanks, building on foundational mathematical research by Robert Ghrist, Justin Curry, and Jakob Hansen.
...
The 0-cochain space ``C^0(\mathcal{F}) = \bigoplus_{v \in V} \mathcal{F}(v)`` collects all vertex-stalk assignments, and the 1-cochain space ``C^1(\mathcal{F}) = \bigoplus_{e \in E} \mathcal{F}(e)`` collects edge-stalk assignments.
...
Equivalently, ``x \in \ker(d) = \ker(L_{\mathcal{F}})``. The space of global sections ``H^0(\mathcal{F}) = \ker(L_{\mathcal{F}})`` captures all globally consistent assignments. Its dimension is the *zeroth sheaf cohomology* of ``\mathcal{F}``.

Use [`nullspace_ldlt`](@ref) to compute a basis for ``H^0(\mathcal{F})`` via sparse LDLt factorization, and [`nearest_global_section`](@ref) to project any 0-cochain to the closest global section (by sheaf energy).
...
If ``L_{II}`` is singular (i.e. the boundary conditions do not uniquely determine the interior values), [`harmonic_extension`](@ref) returns both a minimum-norm particular solution and a basis for the solution's indeterminate directions.

### Dynamics and Control

A *trajectory sheaf* encodes ``k`` steps of the linear discrete-time dynamics ``x_{t+1} = A x_t`` as a sheaf on a path graph of length ``k+1``. Vertex stalks are copies of the state space ``\mathbb{R}^d``, and the restriction maps for each edge ``(t, t+1)`` are:

```math
\rho_{t \trianglelefteq e} = A, \quad \rho_{(t+1) \trianglelefteq e} = I_d.
```

The coboundary condition ``d\, x = 0`` is then exactly ``A x_t - x_{t+1} = 0``, so **global sections are valid trajectories**. Solving a two-point boundary-value problem (BVP) ``x_0 = \bar{x}_0,\; x_k = \bar{x}_k`` reduces to harmonic extension on the interior time steps.

For controlled systems ``\dot{x}(t) = A_c x(t) + B_c u(t)``, the [`ControlledTrajectorySheaf`](@ref) constructs the analogous sheaf using the zero-order-hold discretization ``x_{t+1} = A_d x_t + B_d u_t``. The space of feasible state-control trajectories hitting prescribed endpoints equals the space of global sections of this sheaf.
...
- **[Sheaf Morphisms](@ref)**: building morphisms between sheaves and verifying the naturality condition.

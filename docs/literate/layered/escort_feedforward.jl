...
# Construct Standard Feedback LQR and Feedforward LQR Controllers
lqr_ctrl = LQRController(dyn, DT, Q_lqr, R_lqr)
ff_lqr_ctrl = FeedforwardLQRController(dyn, DT, Q_lqr, R_lqr)
...
# Standard feedback LQR agent configurations
lqr_configs = [(init_states[i], dyns[i], LQRController(dyns[i], DT, Q_lqr, R_lqr).K) for i in 1:NA]

# Feedforward LQR agent configurations
ff_configs  = [(init_states[i], dyns[i], FeedforwardLQRController(dyns[i], DT, Q_lqr, R_lqr)) for i in 1:NA]
...
prob_ff = LayeredControlProblem(;
    sheaf=sheaf,
    target_nodes=[TV1],
    target_trajectory_func=target1_pos,
    target_velocity_func=target1_vel,
    agent_configs=ff_configs,
    dt=DT,
    steps=STEPS,
    r_ring=r_ring
)

# Run Standard Feedback LQR Simulation
init_distributed_agents!(workers_pids, lqr_configs, DT, epsilon)
sim_lqr_res = run_layered_simulation(prob_lqr, workers_pids; mode=:distributed)

# Run Feedforward LQR Simulation
init_distributed_agents!(workers_pids, ff_configs, DT, epsilon)
sim_ff_res = run_layered_simulation(prob_ff, workers_pids; mode=:distributed)
...
# ### 2. Feedforward LQR Escort Animation
animate_layered_escort(sim_ff_res; frame_step = 2, filename = "layered_escort_feedforward.gif", fps = 15)
nothing # hide

# ![Feedforward LQR Escort Tracking](layered_escort_feedforward.gif)

# ### 3. Tracking Lag Elimination Comparison
#
# We compare the per-agent tracking lag $||x_i[1:3] - q^*_i[1:3]||$ over time between 
# standard LQR feedback and feedforward LQR.

ts = (1:STEPS) .* DT

err_lqr = zeros(STEPS, NA)
err_ff  = zeros(STEPS, NA)

for step in 1:STEPS
    for i in 1:NA
        qstar_3d = sim_lqr_res.qstar_history[step, i, 1:3]
        err_lqr[step, i] = norm(sim_lqr_res.sim_data[step, i, 1:3] - qstar_3d)
        err_ff[step, i]  = norm(sim_ff_res.sim_data[step, i, 1:3] - qstar_3d)
    end
end

mean_err_lqr = mean(err_lqr, dims=2)[:]
mean_err_ff  = mean(err_ff, dims=2)[:]
...
plot!(p_comp, ts, mean_err_lqr, lw=2.0, color=:crimson, label="Standard LQR Feedback")
plot!(p_comp, ts, mean_err_ff,  lw=2.0, color=:forestgreen, label="Feedforward + LQR")

# # Direct vs. Pushforward Hierarchical Sheaf Coordination Comparison

# This example compares two formulation paradigms for multi-ring formation coordination:
# 1. **Direct 2-Layer Sheaf Control (escort.jl style):** Harmonic extension solved directly on the full agent sheaf $F$ pinned to Target 1 and Target 2.
# 2. **3-Layer Pushforward Sheaf Control:** High-level harmonic extension solved on coarse pushforward sheaf $f_\star F$, then lifted to $F$.
...
function build_multilayer_sheaf(na1, na2, na3, r1, r2, r3)
    ...
    for i in 1:na3
        ...
        v = r3_agents[i%na3+1]
        # angle_u = (i - 1) * 2π / na3
        # angle_v = (i % na3) * 2π / na3
        # du = [cos(angle_u), sin(angle_u), 0.0] * r3
        # dv = [cos(angle_v), sin(angle_v), 0.0] * r3
        # add_sheaf_edge!(F, u, v, se3_translation_matrix(du), se3_translation_matrix(dv))
        # this should produce a straight line along the axis between the two targets.
    ...
    # Agent 13 (Ring 3) -> Target 1 (T1_NODE)
    # Agent 14 (Ring 3) -> Target 2 (T2_NODE)
...
# --- Direct 2-Layer Harmonic Solver on Sheaf F ---
boundary0 = Dict(T1_NODE => target1_pos(0.0), T2_NODE => target2_pos(0.0))
_, _, H_G, B_G = _harmonic_extension_restricted_laplacian(F, boundary0)
H_G_dense = Matrix(H_G)
B_G_dense = Matrix(B_G)

function solve_direct_harmonic(p1, p2)
    q_flat = H_G_dense \ (-B_G_dense * [p1; p2])
    return reshape(q_flat, 4, 15)'
end
...
for step in 1:STEPS
    ...
    p2 = target2_pos(t)
    p3_mid = 0.5 * (p1 + p2)
    push!(target_history_direct, [p1, p2, p3_mid])

    qstar_agents = solve_direct_harmonic(p1, p2)
...
# Pre-calculate axis limits based on full dataset / last frame so animation limits remain constant
...
# Relative views limits pre-computation
rel_lims = []
...
# Safe error bounds across full simulation
...
function make_comprehensive_frame(step)
    ...
    p1 = plot(aspect_ratio=:equal, title="Direct Sheaf F Top-Down View (t=$(round(t, digits=2))s)",
    ...
    p5 = plot(time_grid[1:step], safe_form_err, yscale=:log10,
        title="Formation Shape Error", xlabel="Time [s]", ylabel="StdDev [m]",
...
savefig(p, "direct_sheaf_comprehensive_snapshot.png")

anim = @animate for step in 1:4:STEPS
    make_comprehensive_frame(step)
end
gif(anim, "direct_sheaf_comprehensive_animation.gif"; fps=10)

println("Direct Sheaf simulation complete.")

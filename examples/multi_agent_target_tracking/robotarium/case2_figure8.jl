...
# multi-agent, single-target tracking simulation for eventual Robotarium use.
#
...
# figure 8 trajectory
function initialize_figure_eight_target(;
    center::Vector{Float64},
    amplitude_x::Float64,
    amplitude_y::Float64,
    angular_speed::Float64,
    initial_phase::Float64 = 0.0
)
    phase = initial_phase

    position = [
        center[1] + amplitude_x * sin(phase),
        center[2] + amplitude_y * sin(2 * phase)
    ]

    # Analytical derivative of the figure-eight trajectory.
    velocity = [
        amplitude_x * angular_speed * cos(phase),
        2 * amplitude_y * angular_speed * cos(2 * phase)
    ]
...
function update_figure_eight_target!(
    target::Target,
    dt::Float64;
    center::Vector{Float64},
    amplitude_x::Float64,
    ...
    # Advance the phase.
    target.phase += angular_speed * dt

    # Keep the phase bounded. This is not mathematically necessary,
    # but avoids allowing it to grow indefinitely.
    target.phase = mod(target.phase, 2π)

    # Update position analytically.
    target.position[1] =
        center[1] +
        amplitude_x * sin(target.phase)

    target.position[2] =
        center[2] +
        amplitude_y * sin(2 * target.phase)

    # Update velocity analytically.
    target.velocity[1] =
        amplitude_x *
        angular_speed *
        cos(target.phase)
...
function update_horizontal_agent!(
    ...
    # Projection onto the tangent direction of the horizontal track.
    P_horizontal = [
        1.0 0.0
        0.0 0.0
    ]

    constrained_control = P_horizontal * unconstrained_control
...
function update_vertical_agent!(
    ...
    unconstrained_control = vertical_sheaf_control(
        sheaf,
        agent,
        target;
        gain = gain
    ...
    # Projection onto the tangent direction of the vertical track.
    P_vertical = [
        0.0 0.0
        0.0 1.0
    ]

    constrained_control = P_vertical * unconstrained_control
    ...
    # Enforce the vertical track.
    agent.position[1] = x_track
    agent.velocity[1] = 0.0
